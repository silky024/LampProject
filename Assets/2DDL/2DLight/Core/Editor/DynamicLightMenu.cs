namespace DynamicLight2D
{
	...
	*/
		...
		static void addCastStencilLight()
		{
			reloadAsset();
			Material m = AssetDatabase.LoadAssetAtPath(_path + "Prefabs/Lights/Materials/CastStencil.mat", typeof(Material)) as Material;
			GameObject gameObject = new GameObject("2DLight_stencil");
			gameObject.transform.position = getCameraPos();
			DynamicLight2D.DynamicLight s = gameObject.AddComponent<DynamicLight2D.DynamicLight>();
			s.Layer.value = AssetUtility.LoadPropertyAsInt("layerMask", tmpAsset);
			s.SolidColor = true;
		...
		static void addCastStencilObject()
		{
			
			reloadAsset ();
			Object prefab = AssetDatabase.LoadAssetAtPath(_path + "Prefabs/Casters/box_stencil.prefab", typeof(GameObject));
	...
	*/
		...
		[MenuItem(menuPath + "/Lit stencil/Point light and Caster", false, 101)]
		static void addCastStencil()
		{
			addCastStencilLight();
			addCastStencilObject();
		}
