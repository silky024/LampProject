namespace DynamicLight2D
{
        ...
        /// </summary>
                ...
                public bool LightIsOverlapped { get; private set; }
		...
		private void initMeshFilterAndRenderer(){
			...
			*/
                        ...
                        for (int i = 0; i < _allMeshes.Length; i++) {
                            if (_allMeshes[i].collider.gameObject.layer == getLayerNumberFromLayerMask()) {
                                if (_allMeshes[i].collider.OverlapPoint(transform.position))
                                {

                                    LightIsOverlapped = true;
                                }
                            }
				...
				{
                    if (LightIsOverlapped) // is emmiter cente point is within a collider, then don't wast generating points. 
                    {
                        LightIsOverlapped = false;
                        Rebuild();
                        return;
                    }
			...
			//-----------------------------------------------------//
				...
				private void GenerateColliderVerts()
				{
						...
						for (int m = 0; m < _colliders.Length; m++)
						{
							...
							if (_colliders[m].collider.collider == null) continue;

							if (_colliders[m].collider.collider.gameObject.layer == getLayerNumberFromLayerMask()) {
								 if (_colliders[m].collider.collider.OverlapPoint((Vector2)transform.position))
								{
										LightIsOverlapped = true;
										return;
								}
				...
				private void GenerateLightVectors()
				{
						...
						if (_sortAngles && !LightIsOverlapped)
