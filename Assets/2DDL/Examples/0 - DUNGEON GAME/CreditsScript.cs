	...
	IEnumerator LoopUpdate () {
		while (true){
			...
			for(int i=0; i< floor.Length; i++){
				if(floor[i].transform.position.x < -(UnityEngine.Camera.main.orthographicSize *2.8f)){ 
			...
			for(int i=0; i< LightsList.Length; i++){
				if(LightsList[i].transform.position.x < -(UnityEngine.Camera.main.orthographicSize *3.2f)){ 
					int tileLastPlace = 0;
